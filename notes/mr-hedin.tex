...
framework \cite{Hedin1965} in which the zeroth-order Hamiltonian is the Dyall Hamiltonian
associated with a CASSCF wave function \cite{Dyall1995}.  The central difficulty is that the
...
the generalized Dyson construction of Hall \cite{Hall1975} and Brouder \cite{Brouder2007} and the first-order
...
Dyall-reference theory once the auxiliary-source representation is adopted.
The connection to Brouder's general correlated-state construction \cite{Brouder2007} and to
Hall's generalized Dyson equation \cite{Hall1975} is established in the literature; the
specialization to an interacting Dyall reference is developed here.
...
\end{equation}
The Dyall Hamiltonian \cite{Dyall1995} is associated with a chosen CASSCF reference and
contains the full interaction within the active orbital space.  In
contrast with the usual Hartree--Fock reference used in conventional
many-body perturbation theory, $\hat H_D$ is therefore itself an
...
\cite{Brouder2007}.  Here we use the same construction algebraically,
but apply it to the exact interacting Dyall generating functional rather
than to a quadratic reference Hamiltonian.
...
The mixed blocks contain
\begin{equation}
  -\frac{i}{2}
\end{equation}
because each corresponding term occurs twice in the cumulant
decomposition of the three-body Green's function,
\begin{equation}
  \boxed{
  \frac{i}{4}\times(-2)
  =
...
The density-dependent contributions contain
\begin{equation}
  -i
\end{equation}
because
...
The genuinely connected three-body contribution has no equivalent
permutation multiplicity remaining after antisymmetrization and
therefore carries
\begin{equation}
  \boxed{
  +\frac{i}{4}.
...
Both contributions vanish independently after the Dyall
simplifications have been made.  Therefore,
\begin{equation}
  \boxed{
  \Sigma_1^{22}=0.
  }
...
In conventional Hedin theory \cite{Hedin1965},
\begin{equation}
  P=-iGG\Gamma
\end{equation}
and
...
vertex-corrected $GW$ \cite{Stefanucci2014,Pavlyukh2026}.
...
higher-order completion, motivated by the general positive-spectral-function
construction of Ref.~\cite{Stefanucci2014}.  Several intermediate identities---in particular the
...
Finally, the number of poles of the Dyall one-particle Green's function
is
\begin{equation}
  \boxed{
  L
