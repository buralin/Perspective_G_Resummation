...
%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Supermatrix formulation: the initial Green's function combined with the MR-RPA screening}
\label{sec:supermatrix}
%%%%%%%%%%%%%%%%%%%%%%%%%

This section derives, in the superoperator language of Sec.~I, the
Hermitian supermatrix that combines the initial Green's function
$\bG_{\rm init}$ of Eq.~\eqref{eq:Ginit} with the screened residual interaction of the
MR-$GW$ method, and lists its matrix elements as implemented in
\texttt{numerics/mrgw\_hermitian} (see \texttt{numerics/equations.pdf} for
all conventions).  The derivation makes explicit which elements are exact,
which are of first order in the residual interaction $\hat V_R$, and where
the $GW$ (quasi-boson) closure enters.  Throughout, $P,Q$ denote inactive
spin orbitals (core $i,j$, virtual $a,b$), $x,y,z,w$ active spin orbitals,
$\mu,\nu$ eigenstates of the active Dyall Hamiltonian with $N_a\pm1$
electrons, $I,J$ MR-RPA modes, and all quantities are real.  The
Hamiltonian partition is $\hat H=\hat H_D+\hat V_R$ of
Eq.~\eqref{eq:global-partition} with the Dyall Hamiltonian of
Ref.~\cite{WangFangLi2026}; $v^R_{pr,qs}=\langle pq|rs\rangle$ without the
all-active elements, $\bar v^R_{pr,qs}=v^R_{pr,qs}-v^R_{ps,qr}$, and
$\gamma_{pq}=\langle\Psi_0|\hat a_p^\dagger\hat a_q|\Psi_0\rangle$ is the
Dyall one-particle density.

\subsection{Retarded propagator in an operator manifold}

\Exact The binary product of Eq.~\eqref{eq:binary-product} is understood
with the anticommutator for the odd operators used below,
\begin{equation}
  (X|Y)=\langle\Psi_0|\,[X^\dagger,Y]_+\,|\Psi_0\rangle ,
  \label{eq:sm-binary}
\end{equation}
and the superoperator Hamiltonian acts as $\hat{\mathcal H}Y=[\hat H,Y]$.
The retarded one-particle propagator is
\begin{equation}
  G_{pq}(z)=\bigl(\hat a_p^\dagger\big|(z-\hat{\mathcal H})^{-1}\big|\hat a_q^\dagger\bigr),
  \qquad z=\omega+i0^+ .
  \label{eq:sm-propagator}
\end{equation}
Equation~\eqref{eq:sm-propagator} reproduces the Lehmann representation of
the retarded propagator: the operators $|m\rangle\langle n|$ built from
exact eigenstates are eigenoperators of $\hat{\mathcal H}$ with eigenvalue
$E_m-E_n$, so that
$(z-\hat{\mathcal H})^{-1}\hat a_q^\dagger=\sum_{mn}\langle m|\hat a_q^\dagger|n\rangle\,|m\rangle\langle n|/[z-(E_m-E_n)]$,
and the anticommutator in Eq.~\eqref{eq:sm-binary} retains exactly the two
terms with $n=0$ ($m$ an $N+1$ state) and $m=0$ ($n$ an $N-1$ state),
\begin{align}
  G_{pq}(z)&=\sum_{m}^{N+1}\frac{\langle\Psi_0|\hat a_p|m\rangle\langle m|\hat a_q^\dagger|\Psi_0\rangle}{z-(E_m-E_0)}
  \nonumber\\
  &+\sum_{n}^{N-1}\frac{\langle\Psi_0|\hat a_q^\dagger|n\rangle\langle n|\hat a_p|\Psi_0\rangle}{z+(E_n-E_0)} .
  \label{eq:sm-lehmann}
\end{align}
The anticommutator is essential: with a commutator the removal term would
change sign.  The identity between Eqs.~\eqref{eq:sm-propagator} and
\eqref{eq:sm-lehmann} is also verified numerically in
\texttt{numerics/run\_checks.py} by explicit construction of the
superoperator resolvent over all pairs of exact eigenstates.

For an orthonormal operator manifold $\{h_k\}$, $(h_k|h_l)=\delta_{kl}$,
the inner projection of Eq.~\eqref{eq:sm-propagator} onto the manifold
gives
\begin{align}
  G(z)&=T\,(z\mathbf 1-\mathbb K)^{-1}\,T^{T},
  \label{eq:sm-inner}\\
  \mathbb K_{kl}&=(h_k|\hat{\mathcal H}|h_l)
  =\langle\Psi_0|\,[h_k^\dagger,[\hat H,h_l]]_+\,|\Psi_0\rangle ,
  \label{eq:sm-K-def}\\
  T_{pk}&=(\hat a_p^\dagger|h_k)=\langle\Psi_0|\,[\hat a_p,h_k]_+\,|\Psi_0\rangle .
  \label{eq:sm-T-def}
\end{align}
Equation~\eqref{eq:sm-inner} is exact if the manifold contains the exact
eigenoperators of $\hat{\mathcal H}$ and is otherwise a Hermitian
approximation: $\mathbb K$ is real symmetric, so $G(z)$ is causal with a
positive-semidefinite spectral function, and its zeroth moment is $TT^{T}$.
All constructions of this section are of this form; they differ only in
the manifold and in the approximations made for the matrix elements
\eqref{eq:sm-K-def}.

\subsection{Top manifold: Eq.~\eqref{eq:Ginit} in the pole basis}
\label{sec:sm-top}

\Exact The manifold of Eq.~\eqref{eq:C1}, supplemented by the exact
charged eigenoperators of the active space, is
\begin{align}
  h_P&=\hat a_P^\dagger\quad(P\in\text{inactive}),
  \label{eq:sm-hP}\\
  h_{\mu+}&=\hat R^{N+1}_\mu=|\Phi_{\rm i}\,\Xi^{N_a+1}_\mu\rangle\langle\Psi_0| ,
  \label{eq:sm-hmu}\\
  h_{\mu-}&=\hat R^{N-1}_\mu=|\Psi_0\rangle\langle\Phi_{\rm i}\,\Xi^{N_a-1}_\mu| ,
  \nonumber
\end{align}
where $|\Phi_{\rm i}\rangle$ is the closed-shell inactive determinant and
$|\Xi^{N_a\pm1}_\mu\rangle$ the eigenstates of the active Dyall Hamiltonian.
The killer condition $\langle\Psi_0|\hat R^{N\pm1}_\mu=0$ of Sec.~I holds by
construction, the manifold is orthonormal,
\begin{align}
  (h_P|h_Q)&=\delta_{PQ},\qquad (h_P|h_{\mu\sigma})=0 ,
  \label{eq:sm-metric}\\
  (h_{\mu\sigma}|h_{\nu\sigma'})&=\delta_{\mu\nu}\delta_{\sigma\sigma'},\nonumber
\end{align}
the last equality being the statement of Sec.~I that no Dyson amplitude
couples the inactive and active spaces directly, and the transition matrix
\eqref{eq:sm-T-def} is
\begin{align}
  T_{QP}&=\delta_{QP},\qquad
  T_{x,\mu+}=\langle\Xi_0|\hat a_x|\Xi^{N_a+1}_\mu\rangle,
  \label{eq:sm-T}\\
  T_{x,\mu-}&=\langle\Xi^{N_a-1}_\mu|\hat a_x|\Xi_0\rangle ,\nonumber
\end{align}
i.e.\ $T=(\mathbf 1_{\rm i}\,|\,\bM)$ with the Dyson amplitudes $\bM$ of
Sec.~I; the active columns are the vectors $d_\alpha$ of
Eq.~\eqref{eq:GD-retarded-spectral} and $TT^{T}=\mathbf 1$ by completeness of
the $N_a\pm1$ sectors, Eq.~\eqref{eq:TD-isometry}.

\Exact The matrix elements \eqref{eq:sm-K-def} with the full Hamiltonian
are as follows.
\begin{align}
  K_{PQ}&=\langle\Psi_0|[\hat a_P,[\hat H,\hat a_Q^\dagger]]_+|\Psi_0\rangle
  =h_{PQ}+\sum_{rs}\bar v_{Pr,Qs}\gamma_{rs}=F_{PQ}
  \nonumber\\
  &=\epsilon_P\delta_{PQ}+[\Sigma^{11}_1]_{PQ},
  \label{eq:sm-KPQ}\\
  K_{\mu\sigma,\nu\sigma'}&=\kappa_{\mu\sigma}\,\delta_{\mu\nu}\delta_{\sigma\sigma'},
  \qquad \kappa_{\mu\pm}=\pm\bigl(E^{N_a\pm1}_\mu-E^{N_a}_0\bigr),
  \label{eq:sm-Kmunu}\\
  K_{P,\mu\sigma}&=\langle\Psi_0|[\hat a_P,[\hat H,\hat R^{N\pm1}_\mu]]_+|\Psi_0\rangle
  \qquad(\text{Eq.~\eqref{eq:Hia} of Sec.~I}).
  \label{eq:sm-KPmu}
\end{align}
Equation~\eqref{eq:sm-KPQ} is the exact first moment of the propagator,
which for inactive indices equals the Dyall Fock operator $F$ of
Ref.~\cite{WangFangLi2026}; after canonicalization of the core and virtual
blocks it reduces to the orbital energies $\epsilon_P$ plus the static
first-order self-energy $\Sigma^{11}_1$ of Eq.~\eqref{eq:sigma11-first},
whose only nonvanishing inactive block is the core--virtual one,
Eq.~\eqref{eq:dyall-sigma11-blocks} (zero for CASSCF orbitals).  This
identifies the block $\mathbf{F}^{\rm HF}_{\rm i,i}$ of Eq.~\eqref{eq:Ginit} with
$F$.  Equation~\eqref{eq:sm-Kmunu} holds because the residual interaction has
the same expectation value, a core constant, in every state
$|\Phi_{\rm i}\Xi\rangle$: its all-active two-body part vanishes by
construction of $v^R$ and its active one-body part is
$u^{\rm eff}_{xy}=[\Sigma^{11}_1]_{xy}=0$,
Eq.~\eqref{eq:active-active-sigma11-zero}.  The block $\bE^{N\pm1}$ of
Eq.~\eqref{eq:Ginit} therefore carries the \emph{signed} pole energies.

\Established The coupling \eqref{eq:sm-KPmu} is of first order in
$\hat V_R$, since $[\hat H_D,\hat R^{N\pm1}_\mu]=\kappa_{\mu\pm}\hat R^{N\pm1}_\mu$
and $(h_P|h_{\mu\sigma})=0$.  Evaluating the double anticommutator with
$\hat V_R=u_{pq}\hat a_p^\dagger\hat a_q+\tfrac14\bar v^R_{pr,qs}\hat a_p^\dagger\hat a_q^\dagger\hat a_s\hat a_r$
gives, for a virtual orbital $a$ and an attachment state,
\begin{align}
  K_{a,\mu+}&=\langle\Psi_0|\hat a_a\hat V_R|\Phi_{\rm i}\Xi^{N_a+1}_\mu\rangle
  \nonumber\\
  &=\sum_z[\Sigma^{11}_1]_{az}T_{z,\mu+}
  +\sum_{zyw}\bar v^R_{az,yw}\,\mathcal C^{+}_{\mu;z,yw},
  \label{eq:sm-Ka}\\
  \mathcal C^{+}_{\mu;z,yw}&=\tfrac12\langle\Xi_0|\hat a_y^\dagger\hat a_w\hat a_z|\Xi^{N_a+1}_\mu\rangle
  -\gamma_{yw}\,T_{z,\mu+},
  \label{eq:sm-Cplus}
\end{align}
and, for a core orbital $i$ and a removal state,
\begin{align}
  K_{i,\mu-}&=-\langle\Phi_{\rm i}\Xi^{N_a-1}_\mu|\hat V_R\,\hat a_i|\Psi_0\rangle
  \nonumber\\
  &=\sum_z[\Sigma^{11}_1]_{iz}T_{z,\mu-}
  +\sum_{zyw}\bar v^R_{iz,yw}\,\mathcal C^{-}_{\mu;z,yw},
  \label{eq:sm-Ki}\\
  \mathcal C^{-}_{\mu;z,yw}&=\tfrac12\langle\Xi^{N_a-1}_\mu|\hat a_y^\dagger\hat a_w\hat a_z|\Xi_0\rangle
  -\gamma_{yw}\,T_{z,\mu-} .
  \label{eq:sm-Cminus}
\end{align}
The first terms are the inactive--active block of the static self-energy
folded with the Dyson amplitudes, $[T^{T}\Sigma^{11}_1T]_{P,\mu\sigma}$; the
second terms are the residues of the first-order Hall blocks
$\Sigma^{12}_1$ and $\Sigma^{21}_1$ of Eqs.~\eqref{eq:sigma12-spectral} and
\eqref{eq:sigma21-spectral}, written with the connected tensors of
Eqs.~\eqref{eq:connected-attachment-tensor} and
\eqref{eq:connected-removal-tensor}.  The factor $\tfrac12$ and the
mean-field subtraction follow from the antisymmetrized interaction without
any further convention; this settles the sign question raised in Sec.~I.
For the pairs of opposite sector (core orbital with an attachment state,
virtual orbital with a removal state) the double anticommutator vanishes
identically, because $\langle\Psi_0|\hat a_i=0$ and $\hat a_a|\Psi_0\rangle=0$.
The corresponding entries of the mixed matrix in Objective~II,
Eqs.~\eqref{eq:KB-Palpha}--\eqref{eq:KB-alphaP}, arise instead from the
first-order change of the Dyson amplitudes induced by $\hat V_R$; they are
required for the exact first-order Green's function (verified numerically
by finite differences on full CI, see \texttt{numerics/run\_checks.py}) and
are included in the implementation by default.

\Proposed Collecting the blocks, the top-space matrix is
\begin{align}
  K&=\operatorname{diag}(\epsilon_P,\kappa_{\mu\sigma})
  +T^{T}\Sigma^{11}_1T
  +\begin{pmatrix}0&K_B\\K_B^{T}&0\end{pmatrix},
  \label{eq:sm-Ktop}\\
  [K_B]_{P,\mu\sigma}&=\sum_{zyw}\mathcal I_{Pz,yw}\,\mathcal C^{\sigma}_{\mu;z,yw},\nonumber
\end{align}
with the bare kernel $\mathcal I=\bar v^R$ (the superoperator evaluation
above) or the statically screened kernel
$\mathcal I_{Pz,yw}=v^R_{Pz,yw}-[W_D(0)]_{Pw,yz}$ of
Eq.~\eqref{eq:mixed-kernel-explicit}, derived in the supermatrix language in
Sec.~\ref{sec:sm-screened}.  The propagator
\begin{equation}
  G_{\rm init}(z)=T\,(z\mathbf 1-K)^{-1}\,T^{T}
  \label{eq:sm-Ginit}
\end{equation}
is Eq.~\eqref{eq:Ginit} written in the pole basis, and the block
inversion of Eq.~\eqref{eq:Ginit2} is its Schur-complement form.

\subsection{MR-RPA bosons and the electron--boson form of \texorpdfstring{$\hat V_R$}{VR}}

\Established The MR-RPA of Sec.~\ref{sec:mr-gw-from-hedin} screens the
residual interaction with the exact Dyall response $P_D$ of
Eq.~\eqref{eq:PD-lehmann}.  Its solution consists of modes $\Omega_I>0$
with amplitudes $X_I,Y_I$ normalized as $X^{T}X-Y^{T}Y=\mathbf 1$, RPA
transition densities $\bar\rho_{I,qs}=\sum_c\rho^{c}_{qs}(X+Y)_{cI}$ built
from the channel transition densities
$\rho^{c}_{qs}=\langle\Phi_c|\hat a_q^\dagger\hat a_s|\Phi_0\rangle$ of the
four channels of Ref.~\cite{WangFangLi2026} (core$\to$virtual,
core$\to$active, active$\to$virtual, active$\to$active), and the couplings
\begin{align}
  M_{pr,I}&=\sum_{qs}v^R_{pr,qs}\,\bar\rho_{I,qs},
  \label{eq:sm-M}\\
  w^{c}_D(z)&=\sum_IM_IM_I^{T}\Bigl[\frac{1}{z-\Omega_I}-\frac{1}{z+\Omega_I}\Bigr],\nonumber
\end{align}
which give the correlation part of $W_D$ of Eq.~\eqref{eq:WD-task3} and the
static kernel $[W_D(0)]=v^R-2\sum_IM_IM_I^{T}/\Omega_I$ used in
Eq.~\eqref{eq:sm-Ktop}.

\Proposed Introduce boson operators $\hat b_I^\dagger$ that create the
MR-RPA modes on the reference, in the quasi-boson approximation
$[\hat b_I,\hat b_J^\dagger]=\delta_{IJ}$, $\hat b_I|\Psi_0\rangle=0$,
$[\hat H,\hat b_I^\dagger]\simeq\Omega_I\hat b_I^\dagger$, and expand the
fluctuation of the residual density in these modes,
$\hat a_q^\dagger\hat a_s-\gamma_{qs}\simeq\sum_I\bar\rho_{I,qs}(\hat b_I+\hat b_I^\dagger)$.
The residual interaction then takes the electron--boson form
\begin{equation}
  \hat V_R\simeq\hat V_R^{\rm stat}
  +\sum_I\sum_{pr}M_{pr,I}\,\hat a_p^\dagger\hat a_r\,(\hat b_I+\hat b_I^\dagger),
  \label{eq:sm-Veb}
\end{equation}
where $\hat V_R^{\rm stat}$ is the one-body mean-field part already
contained in $K$ through $\Sigma^{11}_1$ and $K_B$, and the total
Hamiltonian becomes
$\hat H_{\rm eb}=\hat H_{\rm top}+\sum_I\Omega_I\hat b_I^\dagger\hat b_I+\hat V_{\rm eb}$,
with $\hat H_{\rm top}$ the fermionic Hamiltonian whose matrix in the top
manifold is $K$.  This is the multireference form of Hedin's
electron--boson model~\cite{Hedin1999}; for a single-determinant reference
its one-boson truncation reproduces $G_0W_0$
exactly~\cite{BintrimBerkelbach2021,TolleChan2023}.

\subsection{Statically screened mixed coupling}
\label{sec:sm-screened}

\Proposed The bare coupling of Eqs.~\eqref{eq:sm-Ka}--\eqref{eq:sm-Cminus}
contains the residual interaction once, in the antisymmetrized combination
$\bar v^R_{Pz,yw}=v^R_{Pz,yw}-v^R_{Pw,yz}$ of a direct and a crossed
interaction line between the inactive orbital $P$ and the three active
operators of the connected tensor $\mathcal C$.  The screened variant of the
implementation replaces the crossed line by the statically screened residual
interaction.  Two derivations lead to the same kernel.

\paragraph{Functional derivative of the parent self-energy.}
The parent MR-$GW$ self-energy is $\Sigma^{11}=\Sigma_H[v_R]+iG_DW_D$,
Eq.~\eqref{eq:MRGW-Hedin-form}.  Its particle--hole irreducible kernel at
frozen $W_D$, $\mathcal I_D=i\,\delta\Sigma^{11}/\delta G_D=v_R-W_D^{x}$,
Eq.~\eqref{eq:MR-GW-BSE-kernel-dynamic}, consists of the bare direct term
generated by the Hartree part and of the screened crossed term generated by
the $GW$ exchange-correlation part.  Since the first-order coupling
\eqref{eq:sm-Ka} is instantaneous, $W_D$ is taken at zero frequency,
Eq.~\eqref{eq:static-MR-kernel-orbital}, which gives
\begin{equation}
  \mathcal I_{Pz,yw}=v^R_{Pz,yw}-[W_D(0)]_{Pw,yz}.
  \label{eq:sm-kernel-scr}
\end{equation}

\paragraph{Static elimination of the bosons.}
The same kernel follows from the electron--boson form \eqref{eq:sm-Veb}.
Eliminating the bosons to second order in the coupling in the static limit
(fermionic energy differences neglected against $\Omega_I$) produces an
induced two-body interaction between residual density fluctuations,
\begin{align}
  \hat V^{\rm ind}&=-\sum_I\frac{1}{\Omega_I}\,\hat\rho_I^{\,2},\qquad
  \hat\rho_I=\sum_{pr}M_{pr,I}\,\hat a_p^\dagger\hat a_r ,
  \label{eq:sm-Vind}\\
  v^{\rm ind}_{pr,qs}&=-2\sum_I\frac{M_{pr,I}M_{qs,I}}{\Omega_I}
  =[w^{c}_D(0)]_{pr,qs},
  \nonumber
\end{align}
so that the total static effective interaction is $v^R+v^{\rm ind}=W_D(0)$
of Eq.~\eqref{eq:sm-M}.  Dressing the crossed line of the first-order
coupling with this effective interaction gives
Eq.~\eqref{eq:sm-kernel-scr}.  The direct line is left bare because the
direct boson exchange between the inactive orbital and the active density
fluctuation is already contained dynamically in the bath of
Sec.~\ref{sec:sm-extended}: the path $P\to(n,I)\to\mu$ through a one-boson
state carries the vertices $M_{Pz,I}$ and $M_{yw,I}$ of the direct line,
whereas the crossed pairing $(Pw)(yz)$ is not generated by the bath, whose
vertices \eqref{eq:sm-g} use the $GW$ closure.  Screening the direct line as
well, $\mathcal I=W_D(0)-[W_D(0)]^{x}$ of Eq.~\eqref{eq:full-W-alternative},
would count this exchange twice and is not used.  Because
$W_D(0)=v^R+\mathcal O(\hat V_R^{2})$, the screened coupling differs from the
bare one at second order only and the first-order exactness of
Sec.~\ref{sec:sm-top} is retained, Eq.~\eqref{eq:M1-firstorder-exact-method}.

\paragraph{Matrix elements.}
Inserting $W_D(0)$ into Eq.~\eqref{eq:sm-Ktop},
\begin{align}
  [K^{\rm scr}_B]_{P,\mu\sigma}
  &=\sum_{zyw}\Bigl(\bar v^R_{Pz,yw}+2\sum_I\frac{M_{Pw,I}M_{yz,I}}{\Omega_I}\Bigr)\mathcal C^{\sigma}_{\mu;z,yw}
  \nonumber\\
  &=[K^{\rm bare}_B]_{P,\mu\sigma}
  +\sum_I\frac{2}{\Omega_I}\sum_{w}M_{Pw,I}\sum_{yz}M_{yz,I}\,\mathcal C^{\sigma}_{\mu;z,yw},
  \label{eq:sm-KB-scr}
\end{align}
i.e.\ the boson-exchange correction is a contraction of the inactive--active
block $M_{Pw,I}$ of the screened-interaction couplings with their
active--active block $M_{yz,I}$ and the same connected tensors as the bare
coupling; no additional active-space quantity is needed.  In the
implementation the kernel \eqref{eq:sm-kernel-scr} is formed from $W_D(0)$
over all coupled MR-RPA modes and contracted with $\mathcal C$
(\texttt{mixed='screened'}); the screened $K_B$ enters the top block $K$
and, for the dressed bath of Eq.~\eqref{eq:sm-B}, also the bath blocks
$B_I$, whereas for the diagonal bath it enters the top block only.  The
static one-body part $T^{T}\Sigma^{11}_1T$ is the same in both variants.

\subsection{Extended manifold and the supermatrix}
\label{sec:sm-extended}

\Proposed The manifold is extended by the one-boson operators
\begin{equation}
  h_{nI}=\begin{cases}h_n\,\hat b_I^\dagger&\sigma_n=+1\ (\text{virtual }P,\ \mu+),\\
  h_n\,\hat b_I&\sigma_n=-1\ (\text{core }P,\ \mu-),\end{cases}
  \label{eq:sm-hnI}
\end{equation}
so that $(h_{mI}|h_{nJ})=\delta_{mn}\delta_{IJ}$ and
$(h_m|h_{nI})=0$ on the boson vacuum.  Two-boson operators are not included
(one-boson truncation).  The matrix elements of $\hat{\mathcal H}_{\rm eb}$
are:
\begin{enumerate}
\item top--top: $K$ of Eq.~\eqref{eq:sm-Ktop}, unchanged by the bosons in
  the one-boson truncation;
\item top--bath, from $\hat V_{\rm eb}$ with the $GW$ closure in which the
  one-body transition density between two charged top states is
  factorized through the reference,
  $\langle m|\hat a_p^\dagger\hat a_r|n\rangle\to T_{pm}T_{rn}$ (the
  remaining terms are either contained in $\hat V_R^{\rm stat}$ or are
  vertex corrections beyond $GW$),
  \begin{equation}
    (h_m|\hat{\mathcal H}_{\rm eb}|h_{nI})=\sum_{pr}T_{pm}M_{pr,I}T_{rn}
    \equiv[g_I]_{mn}=[T^{T}M_IT]_{mn};
    \label{eq:sm-g}
  \end{equation}
\item bath--bath: the fermionic Hamiltonian acts on $h_n$ as in the top
  manifold and the boson contributes $\sigma_n\Omega_I$, while
  $\hat V_{\rm eb}$ would connect to two-boson states and is dropped,
  \begin{equation}
    (h_{mI}|\hat{\mathcal H}_{\rm eb}|h_{nJ})=\delta_{IJ}\,[B_I]_{mn},\qquad
    B_I=K+\operatorname{diag}(\sigma_n)\,\Omega_I .
    \label{eq:sm-B}
  \end{equation}
\end{enumerate}
The inner projection \eqref{eq:sm-inner} on the extended manifold gives the
supermatrix and the propagator
\begin{widetext}
\begin{equation}
  \mathbb H=\begin{pmatrix}K&g_1&g_2&\cdots\\ g_1^{T}&B_1&0&\\ g_2^{T}&0&B_2&\\ \vdots&&&\ddots\end{pmatrix},
  \qquad
  \mathbb T=\begin{pmatrix}T&0&0&\cdots\end{pmatrix},
  \qquad
  G(z)=\mathbb T\,(z-\mathbb H)^{-1}\,\mathbb T^{T}
  =T\Bigl[z\mathbf 1-K-\sum_Ig_I\,(z\mathbf 1-B_I)^{-1}g_I^{T}\Bigr]^{-1}T^{T}.
  \label{eq:sm-supermatrix}
\end{equation}
\end{widetext}
The inactive--inactive block of $g_I$, $[g_I]_{PQ}=M_{PQ,I}$, is the
ordinary $GW$ vertex of the inactive space; the inactive--active and
active--active blocks, $[g_I]_{P,\mu\sigma}=\sum_xM_{Px,I}T_{x,\mu\sigma}$ and
$[g_I]_{\mu\sigma,\nu\sigma'}=\sum_{xy}T_{x,\mu\sigma}M_{xy,I}T_{y,\nu\sigma'}$,
are the couplings of the boson states to the active charged states.
Because $\mathbb H$ is real symmetric and $\mathbb T\mathbb T^{T}=TT^{T}=\mathbf 1$,
$G(z)$ is causal, its spectral function is positive semidefinite, its
zeroth moment is exact, and its first moment $TKT^{T}$ does not involve the
bath (cf.\ Eqs.~\eqref{eq:spectral-PSD}, \eqref{eq:zeroth-moment-exact} and
\eqref{eq:M1-main-result}).

\subsection{Limits and relation to the other sections}

\begin{enumerate}
\item \emph{No bosons} ($g_I=0$): Eq.~\eqref{eq:sm-supermatrix} reduces to
  Eq.~\eqref{eq:sm-Ginit}, i.e.\ to Eq.~\eqref{eq:Ginit} of Sec.~I.
\item \emph{Zeroth-order bath}, $B_I\to\operatorname{diag}(\kappa_n+\sigma_n\Omega_I)$,
  and $K_B=0$: the bath Schur complement is the spectral MR-$GW$
  self-energy of Eq.~\eqref{eq:MRGW-spectral-task3} (Eq.~(S52) of
  Ref.~\cite{WangFangLi2026}) in all four channels, and by the
  linearization lemma \eqref{eq:linearization-lemma}
  $G=[G_D^{-1}-\Sigma^{\rm MR\text{-}GW}]^{-1}$ exactly,
  Eqs.~\eqref{eq:MRGW-Dyson-task3} and \eqref{eq:linearization-equals-Dyson}:
  the published MR-$GW$ method.
\item \emph{Zeroth-order bath with $K_B\neq0$}: the common Hermitian matrix
  of Objective~II, Eqs.~\eqref{eq:K-top-final}--\eqref{eq:G-common-final}
  (``MR-$GW$ + mixed'' in the implementation, bare or screened kernel).
\item \emph{Dressed bath}, Eq.~\eqref{eq:sm-B}: the Eq.~\eqref{eq:Ginit}
  Hamiltonian acts inside the one-boson sector.  Without cross-sector
  couplings $[K,\sigma]=0$, and with $K=U\Lambda U^{T}$, $Z_k=TU_{:,k}$,
  \begin{align}
    G(z)&=\bigl[G_{\rm init}(z)^{-1}-\Sigma^{GW}[G_{\rm init}](z)\bigr]^{-1},
    \label{eq:sm-dressed}\\
    \Sigma^{GW}[G_{\rm init}]_{pq}(z)&=\sum_{kI}\frac{[M_IZ_k]_p[M_IZ_k]_q}{z-\lambda_k-\sigma_k\Omega_I},\nonumber
  \end{align}
  the $GW$ convolution \eqref{eq:MRGW-convolution} evaluated with the
  Eq.~\eqref{eq:Ginit} propagator instead of $G_D$; with cross-sector
  couplings Eq.~\eqref{eq:sm-supermatrix} is the definition.  This is the
  first iteration of a $GW_0$-type self-consistency started from
  $G_{\rm init}$ with the top space held fixed (see
  \texttt{notes/mr-gw-selfconsistency.tex}).
\end{enumerate}
The implementation verifies items~2 and~4 as exact identities to
$10^{-14}$, the first-order exactness of item~3 by finite differences on
full CI, and positivity and the zeroth moment for all variants.

\subsection{Matrix elements as implemented}

For reference, the quantities entering Eq.~\eqref{eq:sm-supermatrix} are
computed as follows (all with PySCF routines; details in
\texttt{numerics/equations.pdf}).
\begin{enumerate}
\item $\epsilon_P$: eigenvalues of the core--core and virtual--virtual
  blocks of the Dyall Fock operator $F=h+J[\gamma]-\tfrac12K[\gamma]$ built
  with the CAS density; $h^{\rm eff}_{xy}=h_{xy}+\sum_k[2(xy|kk)-(xk|ky)]$
  defines the active Dyall Hamiltonian.
\item $\kappa_{\mu\pm}$ and $T_{x,\mu\pm}$: dense diagonalization of the
  active Dyall Hamiltonian in the $N_a$, $N_a\pm1$ sectors and overlaps of
  $\hat a_x^{(\dagger)}|\Xi_0\rangle$ with the eigenvectors,
  Eq.~\eqref{eq:sm-T}.
\item $\Sigma^{11}_1=u+\bar v^R\gamma$ with $u=h-h^{D}$, $h^{D}$ the
  one-body part of $\hat H_D$ (inactive diagonal $\epsilon_P$, active block
  $h^{\rm eff}$), Eq.~\eqref{eq:sigma11-first}.
\item $\mathcal C^{\pm}_{\mu;z,yw}$ from the three-operator amplitudes of
  Eqs.~\eqref{eq:sm-Cplus} and \eqref{eq:sm-Cminus}; $K_B$ from
  Eq.~\eqref{eq:sm-Ktop} with $\mathcal I=\bar v^R$ or
  $\mathcal I=v^R-[W_D(0)]^{x}$.
\item MR-RPA: channel densities and energies, direct-RPA matrices
  $A_{cc'}=\omega_c\delta_{cc'}+\sum\rho^{c}v^R\rho^{c'T}$,
  $B_{cc'}=\sum\rho^{c}v^R\rho^{c'}$, solved through
  $(A-B)^{1/2}(A+B)(A-B)^{1/2}$; then $M_{pr,I}$, $W_D(0)$ and
  $g_I=T^{T}M_IT$ from Eq.~\eqref{eq:sm-M}; modes with $M_I=0$ (spin-flip
  modes) are dropped.
\item $B_I$ from Eq.~\eqref{eq:sm-B} (dressed) or its diagonal (MR-$GW$
  form).  The supermatrix is applied matrix-free, selected roots are
  obtained by a Davidson solver with root following by maximal overlap, and
  $G(z)$ by the Schur complement in Eq.~\eqref{eq:sm-supermatrix}.
\end{enumerate}

\subsection{Numerical illustration}

Table~\ref{tab:sm-results} lists the principal ionization and attachment
energies for linear H$_4$ (STO-6G, CASSCF(2,2)) and the three lowest
ionization energies of O$_3$ (6-31G, CASCI in RHF orbitals, the setting of
Ref.~\cite{WangFangLi2026}) for the variants of
Eq.~\eqref{eq:sm-supermatrix}; the scripts are \texttt{numerics/ginit\_prelim.py}
and \texttt{numerics/check\_ozone.py}.

\begin{table*}[t]
\caption{Principal ionization and attachment energies (eV) from the
variants of the supermatrix \eqref{eq:sm-supermatrix}.  ``Eq.~(7)'':
$g_I=0$, bare $K_B$; ``diag.'': zeroth-order bath,
$B_I\to\operatorname{diag}(\kappa_n+\sigma_n\Omega_I)$; ``dressed'':
Eq.~\eqref{eq:sm-B}; ``bare''/``screened'': kernel $\mathcal I=\bar v^R$ or
$v^R-[W_D(0)]^{x}$ in $K_B$; all rows include the cross-sector couplings
unless stated otherwise.  For H$_4$ the HOMO-like IP and LUMO-like EA are given,
for O$_3$ the $^2A_1$, $^2B_2$, $^2A_2$ IPs; full CI (H$_4$) and the DMRG
values of Ref.~\cite{WangFangLi2026} (O$_3$) are the references.}
\label{tab:sm-results}
\begin{ruledtabular}
\begin{tabular}{lccc}
variant & H$_4$ IP/EA & O$_3$(6,4) & O$_3$(8,5) \\
\hline
Eq.~(7), no bath & 11.13/8.56 & 14.36/14.62/15.13 & 13.53/13.82/13.10 \\
MR-$GW$ ($K_B=0$, diag.) & 10.88/8.52 & 12.65/12.94/14.56 & 11.58/12.09/12.94 \\
Eq.~(7) + diag.\ bath, bare & 10.86/8.50 & 12.68/12.90/14.57 & 11.79/12.08/12.94 \\
Eq.~(7) + diag.\ bath, screened & 10.86/8.51 & 12.68/13.21/14.99 & 12.01/12.27/13.32 \\
Eq.~(7) + dressed bath, bare & 10.86/8.50 & 12.68/12.89/14.53 & 11.78/12.05/12.92 \\
Eq.~(7) + dressed bath, screened & 10.85/8.50 & 12.63/13.14/14.93 & 11.98/12.22/13.29 \\
dressed bath, bare, no cross-sector [Eq.~\eqref{eq:sm-dressed}] & 10.86/8.50 & 12.65/12.86/14.52 & 11.72/12.02/12.93 \\
dressed bath, screened, no cross-sector [Eq.~\eqref{eq:sm-dressed}] & 10.86/8.50 & 12.35/12.92/14.54 & 11.71/11.98/12.94 \\
reference & 11.09/8.79 & 12.34/12.59/13.34 & 12.34/12.59/13.34 \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
for further work.
%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Working equations of the extended-Koopmans variants}
\label{sec:ekt-variants}
%%%%%%%%%%%%%%%%%%%%%%%%%

\Proposed The supermatrix of Sec.~\ref{sec:supermatrix} takes the active
space as input in the form of charged poles $(\kappa_{\alpha\sigma},d_{\alpha\sigma})$
with their connected three-operator tensors $\mathcal C^{\sigma}_{\alpha}$,
and of neutral excitations $(\omega_\nu,\rho^{\nu})$ for the active--active
channel of the MR-RPA.  In Sec.~\ref{sec:supermatrix} these are the exact
eigenstates of the active Dyall Hamiltonian $\hat H_A$ in the $N_a\pm1$ and
$N_a$ sectors, whose number is the dimension of the CI sectors.  This
section gives the working equations of the same schemes when the charged
states are replaced by extended-Koopmans (EKT) states
\cite{DaySmithGarrod1974,MorrellParrLevy1975}, without or with the
2h1p/2p1h sector, and the neutral states by extended-RPA (ERPA)
excitations \cite{ChatterjeePernal2012}.  Both replacements are inner
projections of the type \eqref{eq:sm-inner}: Hermiticity, causality and
positivity of the supermatrix are preserved, and the low spectral moments
of the active propagator remain exact.  Throughout, $\hat H_A$ is the
active part of $\hat H_D$ (one-body part $h^{\rm eff}$, all-active
two-electron integrals), $|\Xi_0\rangle$ and $E_0$ its ground state and
energy, $\gamma_{xy}=\langle\Xi_0|\hat a_x^\dagger\hat a_y|\Xi_0\rangle$
and $\Gamma_{xy,wz}=\langle\Xi_0|\hat a_x^\dagger\hat a_y^\dagger\hat a_w\hat a_z|\Xi_0\rangle$
its one- and two-particle density matrices, $x,y,z,w$ active spin
orbitals and $\sigma=\pm$ the sector label.  The implementation is
\texttt{numerics/mrgw\_hermitian/ekt\_erpa.py}; all variants are run by
\texttt{numerics/ginit\_prelim.py} with \texttt{--reference ekt}
(primary manifold) or \texttt{--reference ekt2} (extended manifold).
Section~\ref{sec:ekt-rdm} gives the same equations in terms of the reduced
density matrices of the CAS wave function and examines the truncation of
their cumulant expansion.

\subsection{Charged states: EKT without the 2h1p/2p1h sector}
\label{sec:ekt-primary}

\Established The primary EKT manifold consists of the $N_a\mp1$ states
\begin{equation}
  |\alpha^-\rangle=\sum_xc^{\alpha}_x\,\hat a_x|\Xi_0\rangle,\qquad
  |\alpha^+\rangle=\sum_xc^{\alpha}_x\,\hat a_x^\dagger|\Xi_0\rangle .
  \label{eq:ekt-states}
\end{equation}
Their Gram and Hamiltonian matrices are functionals of $\gamma$ and
$\Gamma$,
\begin{align}
  S^{-}_{xy}&=\langle\Xi_0|\hat a_x^\dagger\hat a_y|\Xi_0\rangle=\gamma_{xy},
  \nonumber\\
  S^{+}_{xy}&=\langle\Xi_0|\hat a_x\hat a_y^\dagger|\Xi_0\rangle=\delta_{xy}-\gamma_{yx},
  \label{eq:ekt-metric}\\
  A^{-}_{xy}&=\langle\Xi_0|\hat a_x^\dagger(\hat H_A-E_0)\hat a_y|\Xi_0\rangle
  =\langle\Xi_0|\hat a_x^\dagger[\hat H_A,\hat a_y]|\Xi_0\rangle ,
  \nonumber\\
  A^{+}_{xy}&=\langle\Xi_0|\hat a_x(\hat H_A-E_0)\hat a_y^\dagger|\Xi_0\rangle
  =\langle\Xi_0|\hat a_x[\hat H_A,\hat a_y^\dagger]|\Xi_0\rangle ,
  \label{eq:ekt-A}
\end{align}
and the EKT states are the solutions of the generalized eigenvalue
problems
\begin{equation}
  A^{\sigma}c^{\alpha}=e_{\alpha\sigma}\,S^{\sigma}c^{\alpha},\qquad
  c^{\alpha T}S^{\sigma}c^{\beta}=\delta_{\alpha\beta},\qquad
  \kappa_{\alpha\sigma}=\sigma\,e_{\alpha\sigma},
  \label{eq:ekt-eig}
\end{equation}
with $e_{\alpha-}$ the ionization and $e_{\alpha+}$ the attachment energies
of the active space and $\kappa_{\alpha\sigma}$ the signed poles of
Sec.~\ref{sec:supermatrix}.  Directions of $S^{\sigma}$ with eigenvalues
below $10^{-8}$ (fully occupied or empty natural orbitals) are removed by
canonical orthogonalization.  The residues are
\begin{align}
  d_{x,\alpha-}&=\langle\alpha^-|\hat a_x|\Xi_0\rangle=[S^{-}c^{\alpha}]_x ,
  \nonumber\\
  d_{x,\alpha+}&=\langle\Xi_0|\hat a_x|\alpha^+\rangle=[S^{+}c^{\alpha}]_x ,
  \label{eq:ekt-d}
\end{align}
and, because $\sum_\alpha c^{\alpha}c^{\alpha T}=(S^{\sigma})^{-1}$ on the
retained space,
\begin{align}
  \sum_{\alpha}d_{\alpha-}d_{\alpha-}^{T}&=\gamma,\qquad
  \sum_{\alpha}d_{\alpha+}d_{\alpha+}^{T}=\mathbf 1-\gamma,
  \label{eq:ekt-M0}\\
  \sum_{\alpha\sigma}\kappa_{\alpha\sigma}\,d_{\alpha\sigma}d_{\alpha\sigma}^{T}
  &=A^{+}-A^{-}=\langle\Xi_0|[\hat a_x,[\hat H_A,\hat a_y^\dagger]]_+|\Xi_0\rangle .
  \label{eq:ekt-M1}
\end{align}
The zeroth and first spectral moments of the active propagator are thus
exact; Eq.~\eqref{eq:ekt-M1} is the double anticommutator
\eqref{eq:sm-K-def} evaluated with $\hat a_x^\dagger,\hat a_y^\dagger$.
The number of poles is at most $2n_{\rm act}$ ($n_{\rm act}$ active spin
orbitals) instead of the dimensions of the $N_a\pm1$ sectors.

\emph{Remark.}  In the language of Sec.~\ref{sec:supermatrix} the EKT is
the sector-resolved inner projection: the removal and the attachment
sector are projected separately on the states generated by the primary
operators, and the transfer operators \eqref{eq:sm-hmu} are built with
$|\alpha^{\pm}\rangle$.  The inner projection on the single operator
manifold $\{\hat a_x^\dagger\}$ with the anticommutator metric
\eqref{eq:sm-binary} would instead give $n_{\rm act}$ poles at the
eigenvalues of $A^{+}-A^{-}$ (the first-moment or Koopmans-like
approximation), which does not resolve the removal and attachment parts
of a partially occupied orbital.

\subsection{Charged states: EKT with the 2h1p/2p1h sector}
\label{sec:ekt-extended}

\Proposed The extended manifold adds the 2h1p and 2p1h operators of the
active space,
\begin{align}
  |\alpha^-\rangle&=\sum_zc^{\alpha}_z\,\hat a_z|\Xi_0\rangle
  +\sum_{y}\sum_{w>z}c^{\alpha}_{ywz}\,\hat a_y^\dagger\hat a_w\hat a_z|\Xi_0\rangle ,
  \nonumber\\
  |\alpha^+\rangle&=\sum_zc^{\alpha}_z\,\hat a_z^\dagger|\Xi_0\rangle
  +\sum_{y}\sum_{w>z}c^{\alpha}_{ywz}\,\hat a_y\hat a_w^\dagger\hat a_z^\dagger|\Xi_0\rangle .
  \label{eq:ekt2-states}
\end{align}
With the manifold vectors $|v_j\rangle$ of one sector collected in a list,
\begin{equation}
  S_{ij}=\langle v_i|v_j\rangle,\qquad
  A_{ij}=\langle v_i|\hat H_A-E_0|v_j\rangle
  \label{eq:ekt2-AS}
\end{equation}
are the Gram and Hamiltonian matrices; their elements involve reduced
density matrices up to fourth order, and the implementation evaluates them
by applying the operators and $\hat H_A$ to CI vectors (PySCF determinant
routines), which avoids the explicit higher-order density matrices.  The
manifold is strongly linearly dependent, so the generalized eigenproblem
is solved after canonical orthogonalization,
\begin{align}
  S&=U\operatorname{diag}(s)U^{T},\qquad
  X=U_{:,s>\tau}\operatorname{diag}(s_{s>\tau})^{-1/2},
  \nonumber\\
  (X^{T}AX)\,\tilde c^{\alpha}&=e_{\alpha\sigma}\,\tilde c^{\alpha},\qquad
  c^{\alpha}=X\tilde c^{\alpha},\qquad
  c^{\alpha T}Sc^{\beta}=\delta_{\alpha\beta},
  \label{eq:ekt2-canon}
\end{align}
with $\tau=10^{-10}\max_js_j$.  Poles and residues follow
Eqs.~\eqref{eq:ekt-eig} and \eqref{eq:ekt-d} with
$d_{x,\alpha\sigma}=\sum_jc^{\alpha}_j\langle v_j|\hat a_x^{(\dagger)}\Xi_0\rangle$,
the rows of $Sc^{\alpha}$ that belong to the primary vectors.

\Exact The extended manifold conserves the spectral moments
$M_0,\ldots,M_3$ of the active propagator.  Since
$(\hat H_A-E_0)\hat a_z|\Xi_0\rangle=[\hat H_A,\hat a_z]|\Xi_0\rangle$ is a
combination of 1h and 2h1p vectors, the projector $\hat P$ on the manifold
satisfies
$\hat P(\hat H_A-E_0)\hat P\,\hat a_z|\Xi_0\rangle=(\hat H_A-E_0)\hat a_z|\Xi_0\rangle$,
and therefore
\begin{equation}
  \langle\Xi_0|\hat a_x^\dagger\bigl[\hat P(\hat H_A-E_0)\hat P\bigr]^{k}\hat a_y|\Xi_0\rangle
  =\langle\Xi_0|\hat a_x^\dagger(\hat H_A-E_0)^{k}\hat a_y|\Xi_0\rangle
  \label{eq:ekt2-moments}
\end{equation}
for $k\le3$ (one factor of $\hat H_A-E_0$ is absorbed on each side), and
likewise for the attachment part.  Whenever the manifold spans the whole
$N_a\mp1$ sector the poles are exact; this is the case for all active
spaces of Table~\ref{tab:ekt-results}, so that the difference between the
extended-manifold results and those of Sec.~\ref{sec:supermatrix} isolates
the effect of the ERPA response.  The number of manifold vectors is
bounded by $n_{\rm act}+n_{\rm act}^{2}(n_{\rm act}-1)/2$ per sector before
the rank reduction.

\subsection{Neutral excitations: ERPA}
\label{sec:erpa}

\Established The active--active channel of the reference polarizability,
Eq.~\eqref{eq:PD-lehmann}, requires the excitation energies $\omega_\nu$
and transition densities
$\rho^{\nu}_{xy}=\langle\nu|\hat a_x^\dagger\hat a_y|\Xi_0\rangle$ of the
$N_a$-electron active space.  In the ERPA they are obtained from the
equation of motion in the manifold of single excitation operators
$\hat E_{pq}=\hat a_p^\dagger\hat a_q$ acting on the correlated reference,
\begin{equation}
  \sum_{rs}\langle\Xi_0|[\hat E_{qp},[\hat H_A,\hat E_{rs}]]|\Xi_0\rangle\,c^{\nu}_{rs}
  =\omega_\nu\sum_{rs}\langle\Xi_0|[\hat E_{qp},\hat E_{rs}]|\Xi_0\rangle\,c^{\nu}_{rs},
  \label{eq:erpa-eom}
\end{equation}
written in the basis of natural spin orbitals of each spin block,
$\gamma=U\operatorname{diag}(n)U^{T}$, in which the metric is diagonal,
$\langle\Xi_0|[\hat E_{qp},\hat E_{rs}]|\Xi_0\rangle=\delta_{pr}\delta_{qs}(n_q-n_p)$.
With the pair manifold
$\mathcal P=\{(p,q):\sigma_p=\sigma_q,\ |n_q-n_p|>10^{-8}\}$, its
excitation part $\mathcal E=\{(p,q)\in\mathcal P:n_q>n_p\}$, the
de-excitation pairs $\bar{\mathcal E}=\{(q,p)\}$, $N_{pq}=n_q-n_p$, and the
double-commutator matrix
\begin{align}
  \mathcal M_{(pq),(rs)}&=\langle\Xi_0|[\hat E_{qp},[\hat H_A,\hat E_{rs}]]|\Xi_0\rangle
  \nonumber\\
  &=\langle\tilde u_{pq}|\hat H_A-E_0|\tilde u_{rs}\rangle
  +\langle\tilde u_{sr}|\hat H_A-E_0|\tilde u_{qp}\rangle ,
  \label{eq:erpa-M}
\end{align}
$|\tilde u_{pq}\rangle=\hat E_{pq}|\Xi_0\rangle$ (symmetrized in the
implementation), Eq.~\eqref{eq:erpa-eom} takes the RPA form
\begin{align}
  \begin{pmatrix}\tilde A&\tilde B\\ \tilde B&\tilde A\end{pmatrix}
  \begin{pmatrix}\tilde X_\nu\\ \tilde Y_\nu\end{pmatrix}
  &=\omega_\nu\begin{pmatrix}\mathbf 1&0\\0&-\mathbf 1\end{pmatrix}
  \begin{pmatrix}\tilde X_\nu\\ \tilde Y_\nu\end{pmatrix},
  \label{eq:erpa-rpa}\\
  \tilde A=N^{-1/2}\mathcal M_{\mathcal E\mathcal E}N^{-1/2},&\qquad
  \tilde B=N^{-1/2}\mathcal M_{\mathcal E\bar{\mathcal E}}N^{-1/2},
  \nonumber
\end{align}
solved as in Sec.~\ref{sec:supermatrix}.  The transition densities of the
normalized solutions ($\tilde X^{T}\tilde X-\tilde Y^{T}\tilde Y=\mathbf 1$)
are
\begin{equation}
  \tilde\rho^{\nu}_{pq}=N_{pq}^{1/2}\tilde X_{\nu,pq},\qquad
  \tilde\rho^{\nu}_{qp}=-N_{pq}^{1/2}\tilde Y_{\nu,pq},\qquad (p,q)\in\mathcal E,
  \label{eq:erpa-rho}
\end{equation}
back-transformed with $U$ to the active spin orbitals.  The ERPA response
fulfils the zeroth and the energy-weighted sum rules of the exact active
response over the single-excitation manifold (verified to $10^{-16}$), and
its number of modes is the number of natural-orbital pairs instead of the
number of CAS excited states.

\Established With the Dyall partition the two remaining active channels of
$P_D$ are products of an inactive orbital and a charged active state, so
that with EKT states they read
\begin{align}
  \text{core}\to\text{active:}&\quad \omega=\kappa_{\alpha+}-\epsilon_i,\qquad \rho_{xi}=d_{x,\alpha+},
  \nonumber\\
  \text{active}\to\text{virtual:}&\quad \omega=\epsilon_a-\kappa_{\alpha-},\qquad \rho_{ax}=d_{x,\alpha-},
  \label{eq:ekt-channels}
\end{align}
i.e.\ the EKT problems shifted by the orbital energies, while the
core$\to$virtual channel is unchanged.  The direct MR-RPA with $v^R$, its
modes $\Omega_I$, the couplings $M_{pr,I}$, the static screened interaction
$W_D(0)$ and the vertices $g_I=T^{T}M_IT$ follow from Eqs.~\eqref{eq:sm-M}
and \eqref{eq:sm-g} without change.  (The implementation also allows the
exact CAS excitations in the active--active channel, which isolates the
effect of the EKT charged manifold.)

\subsection{Working equations of the variants}
\label{sec:ekt-working}

\Proposed With the EKT poles and residues of Sec.~\ref{sec:ekt-primary} or
\ref{sec:ekt-extended} and the MR-RPA of Sec.~\ref{sec:erpa}, the
quantities of Sec.~\ref{sec:supermatrix} are as follows.  The transfer
operators \eqref{eq:sm-hmu} are built with the EKT states,
$h_{\alpha+}=|\Phi_{\rm i}\alpha^+\rangle\langle\Psi_0|$ and
$h_{\alpha-}=|\Psi_0\rangle\langle\Phi_{\rm i}\alpha^-|$; the metric
\eqref{eq:sm-metric} holds by the normalization in
Eq.~\eqref{eq:ekt-eig}, and
\begin{align}
  T&=\bigl(\mathbf 1_{\rm i}\ \big|\ d_{\alpha\sigma}\bigr),\qquad TT^{T}=\mathbf 1,
  \label{eq:ekt-T}\\
  K_{PQ}&=\epsilon_P\delta_{PQ}+[\Sigma^{11}_1]_{PQ},\qquad
  K_{\alpha\sigma,\beta\sigma'}=\kappa_{\alpha\sigma}\delta_{\alpha\beta}\delta_{\sigma\sigma'},
  \label{eq:ekt-Kdiag}\\
  K_{P,\alpha\sigma}&=\sum_x[\Sigma^{11}_1]_{Px}\,d_{x,\alpha\sigma}
  +\sum_{zyw}\mathcal I_{Pz,yw}\,\mathcal C^{\sigma}_{\alpha;z,yw},
  \label{eq:ekt-KB}\\
  \mathcal C^{+}_{\alpha;z,yw}&=\tfrac12\langle\Xi_0|\hat a_y^\dagger\hat a_w\hat a_z|\alpha^+\rangle-\gamma_{yw}\,d_{z,\alpha+},
  \nonumber\\
  \mathcal C^{-}_{\alpha;z,yw}&=\tfrac12\langle\alpha^-|\hat a_y^\dagger\hat a_w\hat a_z|\Xi_0\rangle-\gamma_{yw}\,d_{z,\alpha-},
  \label{eq:ekt-C}
\end{align}
with $\mathcal I=\bar v^R$ (bare) or
$\mathcal I_{Pz,yw}=v^R_{Pz,yw}-[W_D(0)]_{Pw,yz}$ (screened,
Eq.~\eqref{eq:sm-kernel-scr}, with $W_D(0)$ from the MR-RPA of
Sec.~\ref{sec:erpa}).  The active block of Eq.~\eqref{eq:ekt-Kdiag}
follows from Eq.~\eqref{eq:ekt-eig} as Eq.~\eqref{eq:sm-Kmunu} from the
eigenvalue equation, because the residual interaction contributes the same
constant in every state $|\Phi_{\rm i}\alpha^{\sigma}\rangle$; the EKT
states are not eigenstates of $\hat H_A$, and the components of
$(\hat H_A-E_0)|\alpha^{\sigma}\rangle$ outside the manifold are what the
inner projection discards.  Equation~\eqref{eq:ekt-KB} is the double
anticommutator \eqref{eq:sm-KPmu} evaluated with $|\alpha^{\sigma}\rangle$
in place of the eigenstates, which changes nothing in the derivation of
Eqs.~\eqref{eq:sm-Ka}--\eqref{eq:sm-Cminus}.  For the primary manifold the
three-operator amplitudes in Eq.~\eqref{eq:ekt-C} are contractions of
$c^{\alpha}$ with the two-particle density matrix,
\begin{align}
  \langle\alpha^-|\hat a_y^\dagger\hat a_w\hat a_z|\Xi_0\rangle&=\sum_xc^{\alpha}_x\,\Gamma_{xy,wz},
  \nonumber\\
  \langle\Xi_0|\hat a_y^\dagger\hat a_w\hat a_z|\alpha^+\rangle&=\sum_xc^{\alpha}_x\bigl(\delta_{zx}\gamma_{yw}-\delta_{wx}\gamma_{yz}+\Gamma_{yx,wz}\bigr),
  \label{eq:ekt-C-rdm}
\end{align}
so that the whole top block is, like the EKT itself, a functional of the
one- and two-particle density matrices of the CAS wave function; for the
extended manifold the amplitudes are evaluated with the CI vectors of
Eq.~\eqref{eq:ekt2-states}.

The variants are the schemes of Table~\ref{tab:sm-results}, obtained from
the supermatrix \eqref{eq:sm-supermatrix} with the EKT top block:
\begin{enumerate}
\item \emph{Eq.~\eqref{eq:Ginit} with EKT poles} ($g_I=0$):
  $G(z)=T(z\mathbf 1-K)^{-1}T^{T}$ with $K$ of
  Eqs.~\eqref{eq:ekt-Kdiag}--\eqref{eq:ekt-KB} and the bare kernel.
\item \emph{MR-$GW$ on EKT/ERPA input} ($K_B=0$, diagonal bath):
  $B_I\to\operatorname{diag}(\kappa_n+\sigma_n\Omega_I)$, $g_I=T^{T}M_IT$.
  By the linearization lemma \eqref{eq:linearization-lemma} this is the
  MR-$GW$ Dyson equation
  $G=[G_{\rm EKT}^{-1}-\Sigma^{\rm MR\text{-}GW}]^{-1}$ with the EKT
  propagator $G_{\rm EKT}=T(z\mathbf 1-K_0)^{-1}T^{T}$,
  $K_0=\operatorname{diag}(\epsilon_P,\kappa_{\alpha\sigma})$, in place of
  $G_D$ and with the ERPA-based screening in the self-energy
  \eqref{eq:MRGW-spectral-task3}.
\item \emph{Diagonal bath with $K_B$}, bare or screened kernel.
\item \emph{Dressed bath}, $B_I=K+\operatorname{diag}(\sigma_n)\Omega_I$,
  bare or screened kernel; without cross-sector couplings
  Eq.~\eqref{eq:sm-dressed} holds with $G_{\rm init}$ built from the EKT
  top block.
\end{enumerate}
In all cases
\begin{equation}
  G(z)=T\Bigl[z\mathbf 1-K-\sum_Ig_I\,(z\mathbf 1-B_I)^{-1}g_I^{T}\Bigr]^{-1}T^{T},
  \label{eq:ekt-G}
\end{equation}
Eq.~\eqref{eq:sm-supermatrix} with a top block of dimension
$n_{\rm i}+n_{\rm EKT}$ and a bath of dimension $(n_{\rm i}+n_{\rm EKT})K_I$,
$K_I$ the number of coupled MR-RPA modes; the reduction of $n_{\rm EKT}$
from the CI-sector dimensions to $\mathcal O(n_{\rm act})$ (primary) is the
practical motivation for the EKT variants.

\Exact The following properties hold for both manifolds and are verified
in \texttt{numerics/run\_checks.py}.
\begin{enumerate}
\item $G(z)$ is causal with a positive-semidefinite spectral function and
  $M_0=TT^{T}=\mathbf 1$, Eq.~\eqref{eq:ekt-M0}.
\item The first moment $M_1=TKT^{T}$ of the no-bath variant with the bare
  kernel coincides in all blocks with that of the exact reference of
  Sec.~\ref{sec:supermatrix} (to $10^{-15}$ in the implementation): the
  active block by Eq.~\eqref{eq:ekt-M1}, the inactive--active block because
  every $K_{P,\alpha\sigma}$ is linear in $|\alpha^{\sigma}\rangle$ and
  $d_{x,\alpha\sigma}$ is its overlap with $\hat a_x^{(\dagger)}|\Xi_0\rangle$,
  so that $\sum_\alpha K_{P,\alpha\sigma}d_{x,\alpha\sigma}$ contains the
  projector on the manifold acting on a vector inside the manifold.  The
  first-order exactness of Sec.~\ref{sec:sm-top} is therefore inherited.
\item The same-sector elements of $K$ equal the Hamiltonian matrix elements
  between the states $\hat a_P^{(\dagger)}|\Psi_0\rangle$ and
  $|\Phi_{\rm i}\alpha^{\sigma}\rangle$ in the full determinant space, and
  the dressed-bath identity \eqref{eq:sm-dressed} holds without
  cross-sector couplings.
\item The extended manifold conserves $M_0,\ldots,M_3$ of the active
  propagator, Eq.~\eqref{eq:ekt2-moments}, and reproduces the exact poles
  whenever it spans the sector.
\end{enumerate}

\subsection{Numerical illustration}
\label{sec:ekt-numerics}

Table~\ref{tab:ekt-results} lists, for the variants of
Table~\ref{tab:sm-results}, the principal ionization and attachment
energies of linear H$_4$ in the 6-31G basis with CASSCF(4,4), the smallest
setting in which the primary EKT manifold is incomplete (16 of 96 charged
poles) while the extended manifold is complete, and the three lowest
ionization energies of O$_3$ (6-31G, CASCI in RHF orbitals) with the two
EKT references; the exact-reference values for O$_3$ are those of
Table~\ref{tab:sm-results}.  The script is
\texttt{numerics/ginit\_prelim.py}, the outputs are kept in
\texttt{numerics/results/}.

\begin{table*}[t]
\caption{Principal ionization/attachment energies of H$_4$ (6-31G,
CASSCF(4,4)) and the $^2A_1$/$^2B_2$/$^2A_2$ ionization energies of O$_3$
(6-31G, CASCI) in eV from the supermatrix variants of
Table~\ref{tab:sm-results} with the exact active states (``exact''), the
primary EKT manifold (``EKT'') and the extended manifold (``EKT+2h1p''),
both with the ERPA active response.  Full CI (H$_4$) and the DMRG values of
Ref.~\cite{WangFangLi2026} (O$_3$) are the references.}
\label{tab:ekt-results}
\begin{ruledtabular}
\begin{tabular}{lccccccc}
 & \multicolumn{3}{c}{H$_4$ IP/EA} & \multicolumn{2}{c}{O$_3$(6,4)} & \multicolumn{2}{c}{O$_3$(8,5)} \\
variant & exact & EKT & EKT+2h1p & EKT & EKT+2h1p & EKT & EKT+2h1p \\
\hline
Eq.~(7), no bath & 12.18/3.64 & 12.71/3.91 & 12.18/3.64 & 15.27/16.02/15.21 & 14.36/14.62/15.13 & 15.16/16.30/15.28 & 13.53/13.82/13.10 \\
MR-$GW$ ($K_B=0$, diag.) & 12.06/3.42 & 12.59/3.76 & 12.06/3.42 & 13.37/13.94/14.64 & 12.66/12.95/14.57 & 12.88/13.77/15.01 & 11.68/12.13/12.97 \\
Eq.~(7) + diag.\ bath, bare & 12.05/3.27 & 12.58/3.54 & 12.05/3.27 & 13.37/13.94/14.65 & 12.70/12.91/14.58 & 12.88/13.77/15.03 & 11.87/12.11/12.97 \\
Eq.~(7) + diag.\ bath, screened & 12.08/3.78 & 12.62/4.07 & 12.08/3.78 & 13.15/14.24/15.08 & 12.68/13.22/15.00 & 13.25/14.15/15.58 & 12.08/12.31/13.36 \\
Eq.~(7) + dressed bath, bare & 12.04/3.30 & 12.57/3.57 & 12.04/3.30 & 13.37/13.93/14.61 & 12.69/12.90/14.54 & 12.86/13.69/15.01 & 11.86/12.08/12.96 \\
Eq.~(7) + dressed bath, screened & 12.07/3.81 & 12.60/4.10 & 12.06/3.81 & 13.09/14.15/15.02 & 12.63/13.15/14.94 & 13.23/14.08/15.54 & 12.05/12.26/13.32 \\
dressed, bare, no cross-sector & 12.04/3.29 & 12.57/3.57 & 12.04/3.30 & 13.36/13.94/14.61 & 12.66/12.87/14.54 & 12.83/13.70/15.00 & 11.81/12.05/12.97 \\
dressed, screened, no cross-sector & 12.04/3.81 & 12.57/4.10 & 12.04/3.81 & 12.77/13.88/14.63 & 12.34/12.93/14.56 & 12.84/13.68/15.02 & 11.79/12.02/12.98 \\
reference & 12.00/2.99 & 12.00/2.99 & 12.00/2.99 & \multicolumn{2}{c}{12.34/12.59/13.34} & \multicolumn{2}{c}{12.34/12.59/13.34} \\
\end{tabular}
\end{ruledtabular}
\end{table*}

The three references show the same pattern for the variants: the
screened kernel raises the $^2B_2$ and $^2A_2$ energies of O$_3$ by
0.3--0.4~eV relative to the bare one, the dressed bath changes the
diagonal-bath results by at most 0.05~eV, and dropping the cross-sector
couplings with the screened kernel lowers $^2A_1$ by about 0.3~eV, exactly
as in Table~\ref{tab:sm-results}.  The extended manifold reproduces the
exact-reference numbers in every variant to 0.01~eV for H$_4$ and
O$_3$(6,4) and to 0.03--0.10~eV for O$_3$(8,5); since its charged poles
are exact, this residual is the effect of the ERPA response in the
active--active channel (12 of 15 and 20 of 24 exact excitations are
represented for the two ozone active spaces).  The primary manifold is
0.5~eV too high for the H$_4$ ionization energy and 0.3--0.4~eV for the
attachment energy in every variant.  For O$_3$ its Eq.~(7) values lie
0.9--2.4~eV above the CASCI cation energies, because the relaxed
multiconfigurational cation states are poorly represented by
$\{\hat a_x|\Xi_0\rangle\}$; the screening lowers them by about 2~eV and
restores the ordering $^2A_1<{}^2B_2<{}^2A_2$, but they remain
0.7--2~eV above the exact-reference results.  The deficiency of the
primary EKT states is thus not repaired by the screening, the mixed
couplings or the dressed bath, whereas the 2h1p/2p1h extension makes the
EKT/ERPA input a faithful replacement for the exact active states at a
top block of the same size (for larger active spaces the rank of the
extended manifold, not the sector dimension, bounds the number of poles).
For the primary manifold the top block has 62 (64) instead of 102 (154)
rows for O$_3$(6,4) ((8,5)) and the dressed bath $1.5\times10^{4}$ instead
of $5.2\times10^{4}$ ($1.5\times10^{5}$) rows.


\subsection{Density-matrix realization and cumulant truncation}
\label{sec:ekt-rdm}

\Exact All matrix elements of Secs.~\ref{sec:ekt-primary}--\ref{sec:erpa}
are expectation values in $|\Xi_0\rangle$ of products of at most eight
creation and annihilation operators and are therefore functionals of the
spin-orbital reduced density matrices (RDMs)
\begin{equation}
  D^{(k)}_{p_1\cdots p_k,\,q_1\cdots q_k}
  =\langle\Xi_0|\hat a_{p_1}^\dagger\cdots\hat a_{p_k}^\dagger\,
  \hat a_{q_k}\cdots\hat a_{q_1}|\Xi_0\rangle ,\qquad k\le4 ,
  \label{eq:rdm-def}
\end{equation}
with $D^{(1)}=\gamma$ and $D^{(2)}_{pq,rs}=\Gamma_{pq,sr}$.  The reduction
is mechanical.  With
$\hat H_A=\sum_{pq}h_{pq}\hat a_p^\dagger\hat a_q+\tfrac12\sum v_{pr,qs}\hat a_p^\dagger\hat a_q^\dagger\hat a_s\hat a_r$,
$v_{pr,qs}=(pr|qs)$, the commutators with the manifold operators are
\begin{widetext}
\begin{align}
  [\hat H_A,\hat a_y]&=-\sum_qh_{yq}\,\hat a_q-\sum_{qrs}v_{yr,qs}\,\hat a_q^\dagger\hat a_s\hat a_r ,
  \qquad
  [\hat H_A,\hat a_y^\dagger]=\sum_ph_{py}\,\hat a_p^\dagger+\sum_{pqs}v_{py,qs}\,\hat a_p^\dagger\hat a_q^\dagger\hat a_s ,
  \label{eq:comm-1h}\\
  [\hat H_A,\hat a_y^\dagger\hat a_w\hat a_z]&=\sum_ph_{py}\,\hat a_p^\dagger\hat a_w\hat a_z
  -\sum_qh_{wq}\,\hat a_y^\dagger\hat a_q\hat a_z+\sum_qh_{zq}\,\hat a_y^\dagger\hat a_q\hat a_w
  -\sum_{rs}v_{zr,ws}\,\hat a_y^\dagger\hat a_s\hat a_r
  \nonumber\\
  &+\sum_{pqs}v_{py,qs}\,\hat a_p^\dagger\hat a_q^\dagger\hat a_s\hat a_w\hat a_z
  -\sum_{qrs}v_{wr,qs}\,\hat a_y^\dagger\hat a_q^\dagger\hat a_s\hat a_r\hat a_z
  +\sum_{qrs}v_{zr,qs}\,\hat a_y^\dagger\hat a_q^\dagger\hat a_s\hat a_r\hat a_w ,
  \label{eq:comm-2h1p}
\end{align}
\end{widetext}
and $[\hat H_A,\hat a_y\hat a_w^\dagger\hat a_z^\dagger]$ follows by
Hermitian conjugation.  Multiplying from the left with the bra operator
$X_i^\dagger$, moving its annihilators to the right of all creators with
$\hat a_i\hat a_j^\dagger=\delta_{ij}-\hat a_j^\dagger\hat a_i$, and
replacing every normal-ordered string of $k$ creators and $k$
annihilators by $D^{(k)}$ gives the blocks of
Eqs.~\eqref{eq:ekt-A}, \eqref{eq:ekt2-AS}, \eqref{eq:erpa-M} and
\eqref{eq:ekt-C}.  For the primary manifold,
\begin{align}
  A^-_{xy}&=-\sum_qh_{yq}\gamma_{xq}-\sum_{qrs}v_{yr,qs}\,D^{(2)}_{xq,rs},
  \label{eq:Aminus-rdm}\\
  A^+_{xy}&=h_{xy}-\sum_ph_{py}\gamma_{px}+\sum_{qs}v_{xy,qs}\gamma_{qs}
  -\sum_{ps}v_{py,xs}\gamma_{ps}
  \nonumber\\
  &+\sum_{pqs}v_{py,qs}\,D^{(2)}_{pq,sx},
  \label{eq:Aplus-rdm}
\end{align}
the classic EKT expressions; for the extended manifold the metric is
\begin{align}
  S_{x,(ywz)}&=D^{(2)}_{xy,zw},
  \nonumber\\
  S_{(abc),(ywz)}&=D^{(3)}_{bcy,zwa}-\delta_{ay}D^{(2)}_{bc,zw}
  \label{eq:S-rdm}
\end{align}
for the bra operator $\hat a_a^\dagger\hat a_b\hat a_c$, and the
Hamiltonian blocks of the removal sector contain 11 terms (1h--2h1p,
with $D^{(2)}$ and $D^{(3)}$) and 28 terms (2h1p--2h1p, with $D^{(2)}$ to
$D^{(4)}$); the ERPA double commutator has 20 terms with $\gamma$ and
$D^{(2)}$ only.  (The attachment blocks, written with the operators
$\hat a_y\hat a_w^\dagger\hat a_z^\dagger$ of Eq.~\eqref{eq:ekt2-states},
carry additional contraction terms with Kronecker deltas between free
indices, 257 in the 2p1h--2p1h block; they involve the same RDM orders.)
The complete lists, generated by the normal-ordering routine of the
implementation (\texttt{numerics/mrgw\_hermitian/wick.py}), are reproduced
in \texttt{numerics/equations.pdf}.  With the exact RDMs this realization
reproduces the CI-vector realization of Secs.~\ref{sec:ekt-primary}--\ref{sec:erpa}
to $10^{-12}$ in all matrices and in $G(z)$ of every variant
(\texttt{numerics/run\_checks.py}).

\Established The RDMs are decomposed into cumulants with the Grassmann
(shuffle) product $\wedge$, in which every distinct assignment of the
upper and of the lower indices to the factors appears once with the sign
of the permutation,
\begin{align}
  D^{(2)}&=\tfrac1{2!}\gamma^{\wedge2}+\lambda_2,\qquad
  D^{(3)}=\tfrac1{3!}\gamma^{\wedge3}+\gamma\wedge\lambda_2+\lambda_3,
  \nonumber\\
  D^{(4)}&=\tfrac1{4!}\gamma^{\wedge4}+\tfrac1{2!}\gamma^{\wedge2}\wedge\lambda_2
  +\tfrac1{2!}\lambda_2\wedge\lambda_2+\gamma\wedge\lambda_3+\lambda_4 ,
  \label{eq:cumulants}
\end{align}
where $\tfrac12\gamma^{\wedge2}_{pq,rs}=\gamma_{pr}\gamma_{qs}-\gamma_{ps}\gamma_{qr}$.
The cumulants are the connected parts of the RDMs: they vanish for a
single determinant, and all elements with indices on two non-interacting
subsystems vanish (both verified numerically, together with the trace
relation $\sum_r\lambda_{2,pr,qr}=(\gamma^2-\gamma)_{pq}$).

\Proposed Truncating the expansion defines approximate inputs for the
extended manifold.  Setting $\lambda_4=0$ changes only the 2h1p--2h1p and
2p1h--2p1h Hamiltonian blocks; setting $\lambda_3=0$ in addition changes
the metric of these blocks, the 1h--2h1p blocks and the three-operator
amplitudes of the mixed coupling, whereas the primary EKT and the ERPA,
which involve $\gamma$ and $D^{(2)}$ only, are unaffected.  The
cumulant-reconstructed $D^{(3)}$ and $D^{(4)}$ are not $N$-representable,
so the Gram matrix of the manifold need not be positive; the canonical
orthogonalization retains only the directions with eigenvalues above
$\tau\max_js_j$, and the relative threshold $\tau$ acts as a
regularization.

Table~\ref{tab:cumulant-results} compares the variants of
Sec.~\ref{sec:ekt-working} for the extended manifold with exact RDMs,
with $\lambda_4=0$, and with $\lambda_3=\lambda_4=0$
(\texttt{numerics/cumulant\_comparison.py}).  The relative norms of the
cumulants are $\|\lambda_k\|/\|D^{(k)}\|=0.14$, $0.04$, $0.015$ for
$k=2,3,4$ (H$_4$), $0.15$, $0.04$, $0.02$ (O$_3$(6,4)) and $0.11$, $0.025$, $0.010$ (O$_3$(8,5)).  Dropping $\lambda_4$ leaves the number of
poles, the metric and the low-lying poles intact: for H$_4$ the two lowest
ionization and attachment poles move by 2--8~meV and every variant
changes by at most 4~meV, while the highest poles of the manifold move by
up to 30~eV and the active spectral moments $M_2$, $M_3$ deviate by
$5\times10^{-4}$~Ha; for O$_3$ the three cation energies change by at most 2~meV for (6,4) and, for (8,5), by 2~meV ($^2A_1$, $^2B_2$) and by 0.10~eV ($^2A_2$), the $^2A_2$ pole itself moving from 13.10 to 13.01~eV.  Dropping $\lambda_3$ as well is
qualitatively different: with the default threshold $\tau=10^{-10}$ the
Gram matrix has 134 negative eigenvalues (H$_4$), the manifold acquires
226 instead of 96 poles, 70 of them on the wrong side of the Fermi level,
and the MR-RPA built from these poles is unstable (164 complex or
negative-norm modes), so no supermatrix can be formed.  With
$\tau=5\times10^{-2}$ the manifold is reduced to 48 poles with the
correct signs, but the ionization and attachment energies are
0.4--0.5~eV too high in every variant, i.e.\ worse than the primary
manifold with the exact $D^{(2)}$; for O$_3$ the raw truncation gives 144 (238) negative metric eigenvalues, 68 (148) wrong-sign poles and 1021 (2736) lost MR-RPA modes for (6,4) ((8,5)), and the regularized manifold raises the cation energies by 0.1--0.7~eV and loses the residue of one of the three cation states entirely ($^2A_2$ for (6,4), $^2B_2$ for (8,5); dashes in Table~\ref{tab:cumulant-results}).  The three-body cumulant is
therefore required for the 2h1p/2p1h sector, while the four-body cumulant
can be dropped at the meV level for the low-lying poles; this is the same
pattern as in second-order multireference perturbation theories, where
the $\lambda_4$ contribution to the doubles blocks is small and routinely
approximated whereas the metric requires the exact 3-RDM.

\begin{table*}[t]
\caption{Extended EKT manifold with the RDM realization: exact RDMs,
$\lambda_4=0$, and $\lambda_3=\lambda_4=0$ with the metric threshold
$\tau=5\times10^{-2}$ (the untruncated $\tau$ gives an unstable
reference, see the text).  Principal IP/EA of H$_4$ (6-31G, CASSCF(4,4))
and $^2A_1$/$^2B_2$/$^2A_2$ IPs of O$_3$ (6-31G, CASCI) in eV; ERPA
response throughout.  Full CI (H$_4$) and DMRG (O$_3$) as reference; a dash marks a cation state whose residue vanishes in the regularized manifold.}
\label{tab:cumulant-results}
\begin{ruledtabular}
\begin{tabular}{llccc}
variant & RDMs & H$_4$ IP/EA & O$_3$(6,4) & O$_3$(8,5) \\
\hline
Eq.~(7), no bath & exact & 12.18/3.64 & 14.36/14.62/15.13 & 13.53/13.82/13.10 \\
 & $\lambda_4=0$ & 12.18/3.64 & 14.36/14.62/15.13 & 13.54/13.82/13.01 \\
 & $\lambda_3=\lambda_4=0$, $\tau$ & 12.66/4.04 & 14.48/14.74/-- & 14.50/--/13.88 \\
MR-$GW$ ($K_B=0$, diag.) & exact & 12.06/3.42 & 12.66/12.95/14.57 & 11.68/12.13/12.97 \\
 & $\lambda_4=0$ & 12.06/3.42 & 12.66/12.95/14.57 & 11.68/12.13/12.88 \\
 & $\lambda_3=\lambda_4=0$, $\tau$ & 12.55/3.85 & 12.75/13.07/-- & 12.30/--/13.70 \\
Eq.~(7) + diag.\ bath, screened & exact & 12.08/3.78 & 12.68/13.22/15.00 & 12.08/12.31/13.36 \\
 & $\lambda_4=0$ & 12.08/3.78 & 12.68/13.22/15.00 & 12.09/12.31/13.26 \\
 & $\lambda_3=\lambda_4=0$, $\tau$ & 12.57/4.19 & 12.75/13.34/-- & 12.75/--/14.10 \\
Eq.~(7) + dressed bath, bare & exact & 12.04/3.30 & 12.69/12.90/14.54 & 11.86/12.08/12.96 \\
 & $\lambda_4=0$ & 12.04/3.30 & 12.69/12.90/14.54 & 11.86/12.08/12.87 \\
 & $\lambda_3=\lambda_4=0$, $\tau$ & 12.53/3.70 & 12.79/13.02/-- & 12.42/--/13.68 \\
Eq.~(7) + dressed bath, screened & exact & 12.06/3.81 & 12.63/13.15/14.94 & 12.05/12.26/13.32 \\
 & $\lambda_4=0$ & 12.06/3.81 & 12.63/13.15/14.94 & 12.06/12.26/13.23 \\
 & $\lambda_3=\lambda_4=0$, $\tau$ & 12.55/4.22 & 12.69/13.27/-- & 12.73/--/14.06 \\
reference & & 12.00/2.99 & 12.34/12.59/13.34 & 12.34/12.59/13.34 \\
\end{tabular}
\end{ruledtabular}
\end{table*}
